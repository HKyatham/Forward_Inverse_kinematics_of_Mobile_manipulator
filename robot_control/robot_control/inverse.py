...
z = []  
q_array = []
for t in np.linspace(90, 450, num=200):
    Vx = -0.1 * sin(t*(pi/180)) * (2*(pi/200))
    Vy = 0
    Vz = 0.1 * cos(t*(pi/180)) * (2*(pi/200))
    Wx = 0
    Wy = 0
    Wz = 0
    ...
    x.append(T_final[3])                           # Appending X-component to the list                   
    y.append(T_final[7])                           # Appending Y-component to the list
    z.append(T_final[11])
    if t < 180:
        circle.scatter(T_final[3], T_final[7],T_final[11])
        plt.ylim(0,1)   
        plt.pause(0.5)
        circle.set_xlabel('X-axis')
        circle.set_ylabel('Y-axis')
        circle.set_zlabel('Z-axis')
        circle.set_title('Circular Trajectory', fontsize = 14, fontweight ='bold')
    # plt.xlim(-0.15,0.15)    
    ...
    q1 = q1 + (q1_dot*delta_t)
    q2 = q2 + (q2_dot*delta_t)
    q3 = q3 + (q3_dot*delta_t)
    q4 = q4 + (q4_dot*delta_t)
    q5 = q5 + (q5_dot*delta_t)
    q6 = q6 + (q6_dot*delta_t)
    ...
    q6 = q6.evalf(3)  
    if t < 180:
        q_array.append([0.0,0.0,float(q1), float(q2), float(q3), float(q4), float(q5), float(q6)])
    # print(q_array)
    # print("Printing q3")
    # print(q3)
